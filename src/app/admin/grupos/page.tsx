...
export default function GruposPage() {
  ...
  const [conglomeradoId, setConglomeradoId] = useState('')
  ...
  const loadData = async () => {
    try {
      const [gruposRes, conglomeradosRes] = await Promise.all([
        supabase
          .from('grupos')
          .select(`
            *,
  ...
  const handleEdit = (row: any) => {
    setEditando(row)
    setNome(row.nome)
    setConglomeradoId(row.conglomerado_id)
    setAtivo(row.ativo)
    setErro('')
    setShowModal(true)
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editando) {
        const { error } = await supabase
          .from('grupos')
          .update({ nome, conglomerado_id: conglomeradoId, ativo })
          .eq('id', editando.id)
        ...
        const { error } = await supabase
          .from('grupos')
          .insert({ nome, conglomerado_id: conglomeradoId, ativo })
          ...
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <select
                  value={conglomeradoId}
                  onChange={(e) => setConglomeradoId(e.target.value)}
                  className="input-field"
                  required
                >
